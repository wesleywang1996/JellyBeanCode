...
class NumJellyEstimator:
    ...
    def set_world_pop(self, people):
        ...
        assert type(people) is float, \
            "Error: World Population must be set to float"
 
        # NE24: Add a test for value her
        if(people <= 0):
            print("\nError: World Population must be greater than 0. \n")
        sys.exit() 
    ...
    def set_frac_ppl_loving_pink(self, frac):
        ...
        # NE24: Add a test for type here
        assert type(frac) is float, \
            "Error: Fraction of pink lovers must be set to float."

        # NE24: Add a test for value here
        if((frac<=0.0) or (frac >= 1.0)):
            print("\n Fraction of people who love pink must be"\
                  +" between 0.0 and 1.0.\n") 
        sys.exit()
